...
World.prototype.getCamera = function () {
  ...
  // near clip 0.001 (lets you zoom in close), far clip 100 (lets you zoom
  // out far past the default starting view without cells getting culled).
  return new THREE.PerspectiveCamera(75, aspectRatio, 0.001, 100);
}
...
World.prototype.getControls = function () {
  ...
  // Override TrackballControls' default center-zoom on the wheel with
  // zoom-to-cursor: keep the world point under the mouse fixed while we
  // scale the camera-to-target distance. Registered in the capture phase
  // so it runs before TrackballControls' own wheel handler.
  ...
  window.addEventListener('wheel', function (e) {
    ...
    var before = screenToWorldCoords(pos);
    // Scale the eye vector (camera - target), the way trackball does.
    // Use deltaY magnitude so fast scrolls zoom more and slow scrolls zoom
    // less, the way map zooms feel native. Clamp deltaY to avoid huge
    // jumps from a single fling on macOS trackpads.
    var sensitivity = 1.0015;        // smaller = gentler zoom per wheel pixel
    var d = Math.max(-100, Math.min(100, e.deltaY));
    var factor = Math.pow(sensitivity, d);
    var eye = camera.position.clone().sub(controls.target);
    eye.multiplyScalar(factor);
    // Respect minDistance / maxDistance if the user has set them
    var len = eye.length();
    if (controls.maxDistance && len > controls.maxDistance) eye.setLength(controls.maxDistance);
    if (controls.minDistance && len < controls.minDistance) eye.setLength(controls.minDistance);
    camera.position.copy(controls.target).add(eye);
    camera.updateMatrixWorld(true);
    // World point under cursor after zoom — pan to keep it under the cursor
    var after = screenToWorldCoords(pos);
...
World.prototype.init = function () {
  ...
  // Point the trackball target straight below the camera so off-center start
  // positions don't tilt the view toward (0,0,0).
  this.controls.target.set(loc.x, loc.y, 0);
  this.camera.lookAt(loc.x, loc.y, 0);
...
Picker.prototype.onMouseUp = function (e) {
  ...
  // else we're in pan mode; zoom in if the camera is far away.
  // Modal popup on click is disabled — originals are not shipped and the
  // 128 px thumbs don't read well at modal size.
  if (world.camera.position.z > config.pickerMaxZ) {
...
Modal.prototype.showCells = function (cellIndices, cellIdx) {
  ...
  // conditionalize the path to the image. The data/originals/ folder is not
  // shipped to GitHub Pages (213k full-res JPGs), so fall back to the 128 px
  // thumbnail. If you later host originals on a CDN, swap this path back.
  var src = config.data.dir + '/thumbs/' + filename;
...
Welcome.prototype.updateProgress = function () {
  ...
  if (progress == 100 &&
    ...
    world.heightmap &&
    !this.started) {
    // auto-start: when everything is loaded, fade the loader and bring the
    // visualization in. No "Enter" button — the transition runs itself.
    this.started = true;
